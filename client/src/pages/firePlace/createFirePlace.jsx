...
import { createfirePlace } from "../../models/firePlace";

export default function CreatefirePlaceForm() {
  const [formData, setFormData] = useState();
  const [info, setinfo] = useState();
  const navigate = useNavigate();

  const postForm = async () => {
    const data = await createfirePlace(formData);
    if (data.status === 201) return navigate(`/created-firePlace/${data.payload._id}`);
  ...
  return (
    <>
      ...
      <form>
        ...
        <button onClick={handleButton}>Create new firePlace</button>
      </form>
      <Link to={"/"}>
        <p>Go back</p>
      </Link>
